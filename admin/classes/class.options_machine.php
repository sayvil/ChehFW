...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
	    ...
	    $options_data = get_option(OPTIONS);
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					...
					if(isset($options_data[$value['id']]))
						$t_value = stripslashes($options_data[$value['id']]);
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {			
						$output .= '<option id="' . $select_ID . '" value="'.$option.'" ' . selected($s_value, $option, false) . ' />'.$option.'</option>';	 
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						if(isset($options_data[$value['id']]))						
							$ta_value = stripslashes($options_data[$value['id']]);			
				...
				case "radio":
					 ...
					 foreach($value['options'] as $option=>$name) {
						$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($options_data[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label><br/>';				
					}
				break;
				...
				case 'checkbox':
					...
					$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($options_data[$value['id']], 1, false) .' />';
				break;
				
				//multiple checkbox option
				case 'multicheck': 			
					//$multi_stored = $options_data[$value['id']];
					(isset($options_data[$value['id']]))? $multi_stored = $options_data[$value['id']] : $multi_stored="";
				...
				case 'color':
					$color_value = '';
					if(isset($options_data[$value['id']]))
						$color_value = $options_data[$value['id']];		
					$output .= '<div id="' . $value['id'] . '_picker" class="colorSelector"><div style="background-color: '.$color_value.'"></div></div>';
					$output .= '<input class="of-color" name="'.$value['id'].'" id="'. $value['id'] .'" type="text" value="'. $color_value .'" />';
				break;
				
				//typography option	
				case 'typography':
				
					$typography_stored = isset($options_data[$value['id']]) ? $options_data[$value['id']] : $value['std'];
				...
				case 'border':
					...
					$border_stored['color'] = '';
					if(isset($options_data[$value['id']]))
						$border_stored = $options_data[$value['id']];
				...
				case 'images':
					...
					$select_value = '';
					if(isset($options_data[$value['id']])) 
						$select_value = $options_data[$value['id']];
				...
				case 'tiles':
					...
					$i = 0;
					$select_value = isset($options_data[$value['id']]) && !empty($options_data[$value['id']]) ? $options_data[$value['id']] : '';
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					$output .= '<textarea id="export_data" rows="8">'.base64_encode(serialize($options_data)) /* 100% safe - ignore theme check nag */ .'</textarea>'."\n";
