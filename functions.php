...
/* ------------------------------------------------------------------------ */
	...
	load_theme_textdomain( 'energy', get_template_directory() . '/framework/languages' );
	load_theme_textdomain( 'energy-framework', get_template_directory() . '/framework/languages' );
	...
	function energy_register_required_plugins() {
		$plugins = array(
			array(
				...
				'source'   				=> get_template_directory_uri() . '/framework/recommended-plugins/post-types-order.zip', // The plugin source
			...
			array(
				...
				'source'   				=> get_template_directory_uri() . '/framework/recommended-plugins/cf-post-formats.zip', // The plugin source
			...
			array(
            	'name'      => 'Contact Form 7',
            	'slug'      => 'contact-form-7',
				'source'   				=> get_template_directory_uri() . '/framework/recommended-plugins/contact-form-7.zip', // The plugin source
				'required' 				=> true, // If false, the plugin is only 'recommended' instead of required
				'version' 				=> '', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
				'force_activation' 		=> false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
				'force_deactivation' 	=> false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
            ...
            array(
            	...
            	'slug'      => 'revslider',
				'source'   				=> get_template_directory_uri() . '/framework/recommended-plugins/revslider.zip', // The plugin source
				'required' 				=> true, // If false, the plugin is only 'recommended' instead of required
				'version' 				=> '', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
				'force_activation' 		=> true, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
				'force_deactivation' 	=> true, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
		...
		$theme_text_domain = 'energy-framework';
...
/* ------------------------------------------------------------------------ */
	...
	function energy_comment( $comment, $args, $depth ) {
	   ...
	   <div id="comment-<?php comment_ID(); ?>" class="comment-body clearfix"> 
	         ...
	         <div class="comment-text">
				 ...
				 <div class="author">
				 	<h4><?php printf( __( '%s', 'energy'), get_comment_author_link() ) ?>-</h4>
				 	<h6><?php comment_reply_link(array_merge( $args, array('depth' => $depth, 'max_depth' => $args['max_depth']))) ?></h6>
				 	<div class="date">
				 	<?php printf(__('%1$s at %2$s', 'energy'), get_comment_date(),  get_comment_time() ) ?></a><?php edit_comment_link( __( '(Edit)', 'energy'),'  ','' ) ?></div>  
				 </div>
		         ...
		         <em><?php _e( 'Your comment is awaiting moderation.', 'energy' ) ?></em>
		         <br />
		      	<?php endif; ?>
   ...
   function pagination($pages = '', $range = 4) {
		...
		if(1 != $pages) {
			if($paged > 2 && $paged > $range+1 && $showitems < $pages) echo "<a href='".get_pagenum_link(1)."'>&laquo; " . __('First', 'energy') . "</a>";
			if($paged > 1) echo "<a href='".get_pagenum_link($paged - 1)."'>&lsaquo; " . __('Previous', 'energy') . "</a>";
			...
			if ($paged < $pages) echo "<a href=\"".get_pagenum_link($paged + 1)."\">" . __('Next', 'energy') . " &rsaquo;</a>";
			if ($paged < $pages-1 &&  $paged+$range-1 < $pages && $showitems < $pages) echo "<a href='".get_pagenum_link($pages)."'>" . __('Last', 'energy') . " &raquo;</a>";
		}
	}
